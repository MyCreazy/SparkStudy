package com.tjh.sparkstudy.hbase
...
/**
  * Created by tjh.
  * Date: 2019/1/3 上午11:18
  **/
object HbaseOpt {
  val configuration = HBaseConfiguration.create()
  val connection = ConnectionFactory.createConnection(configuration)
  val admin = connection.getAdmin
  val table = connection.getTable(TableName.valueOf("snow:test1"))
  ...
  def scanDataFromHTable(columnFamily: String, column: String) = {
    ...
    while (result != null) {
      ////清洗数据到hive
      var hbaseStr=Bytes.toString(result.getValue(Bytes.toBytes(columnFamily), Bytes.toBytes(column)));
      println(hbaseStr)
      var acardResult= hbaseStr.split(",")
